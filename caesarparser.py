...
class CaesarParser:
    ...
    def _get_fields_by_name_from_characteristics_lists(self, characteristics: dict[str, Any], field_names: set[str]) -> Iterable[dict[str, str]]:
    ...
    def parse_default_characteristics_lists(self, item_list: dict[str, Any]) -> Iterable[dict[str, str]]:
        field_names = {"Waarde", "Level ( J / M / S / E )"}
        yield from self._get_fields_by_name_from_characteristics_lists(item_list, field_names)

    def parse_education(self, item_list: dict[str, Any]) -> Iterable[tuple[str, str]]:
        field_names = {"Profiel opl naam", "Profiel opl start", "Profiel opl eind", "Profiel opl diploma"}
        yield from self._get_fields_by_name_from_characteristics_lists(item_list, field_names)
            
    def parse_charachteristics_lists(self, characteristics_lists: list[dict[str, Any]]) -> Iterable[tuple[str, str]]:
    ...
    def parse_courses(self, item_list: dict[str, Any]) -> Iterable[tuple[str, str]]:
        field_names = {"Value", "Profiel cur naam", "Profiel cur certificaat", "Profiel cur eind"}
        cursussen: list = self._get_fields_by_name_from_characteristics_lists(item_list, field_names)
        yield from cursussen
    
    def parse_experiences(self, item_list: dict[str, Any]) -> Iterable[tuple[str, str]]:
        field_names = {"Value", "Profiel ervaring klant", "Profiel ervaring branche", "Profiel ervaring datum vanaf",
                       "Profiel ervaring datum tot", "Profiel ervaring functie",  "Show on CV", "Profiel ervaring methoden",
